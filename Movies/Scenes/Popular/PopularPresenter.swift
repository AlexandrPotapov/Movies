...
protocol PopularViewPresenterProtocol {
...
class PopularPresenter: PopularViewPresenterProtocol {
  ...
  required init(view: PopularViewProtocol, dataFetcher: DataFetcher, localStorage: MoviesStoreProtocol) {
    self.view = view
  ...
  func getPopularMovies() {
    ...
    dataFetcher.getPopularResponse(page: "\(pageCount)") { response in
